...
class ResultRepository(private val resultDao: ResultDao, commandTemplateDao: CommandTemplateDao, private val context: Context) {
    val KUPIDO2_SEARCH = "KUPIDO2_SEARCH"
...
//        }
    ...
    private suspend fun getYoutubeVideo(inputQuery: String, resetResults: Boolean, addToResults: Boolean) : List<ResultItem>{
        ...
        if (resetResults) {
            ...
            res.filter { it.playlistTitle.isBlank() }.forEach { it.playlistTitle = KUPIDO2_SEARCH }
        }
        if (addToResults){
            val ids = resultDao.insertMultiple(res)
            ids.forEachIndexed { index, id ->
    ...
    private suspend fun getFromYTDLP(inputQuery: String, resetResults: Boolean, addToResults: Boolean, singleItem: Boolean = false) : List<ResultItem> {
        ...
        ytdlpUtil.getFromYTDL(inputQuery, singleItem) { results ->
            ...
            results.filter { it.playlistTitle.isBlank() }.forEach { it.playlistTitle = KUPIDO2_SEARCH }
            if (addToResults) {
    ...
    suspend fun updateDownloadItem(
        ...
        if (downloadItem.needsDataUpdating()){
            runCatching {
                ...
                if (downloadItem.playlistTitle.isEmpty() || (downloadItem.playlistTitle.isNotBlank() && downloadItem.playlistTitle != KUPIDO2_SEARCH)   ) downloadItem.playlistTitle = info.playlistTitle
