...
class MainActivity : BaseActivity() {
    ...
    private fun handleIntents(intent: Intent) {
        if (intent.getBooleanExtra("RESTORE_PLAYER", false)) {
            val navHostFragment = supportFragmentManager.findFragmentById(R.id.frame_layout) as androidx.navigation.fragment.NavHostFragment
    ...
    private fun askAutoUpdatePreferences() {
        ...
        updateAppLayout.isVisible = false
    ...
    private fun callAutoUpdates(firstRun : Boolean = false) {
        if (false && preferences.getBoolean("update_app", false)) {
